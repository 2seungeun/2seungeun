...
비디오 생성, 이해, video-language는 로봇으로 이어지는 방식이 서로 달랐다. 같은 생성 계열 안에서도 미래 장면을 만드는 모델과 action을 함께 내놓는 모델은 로봇에서 다른 역할을 맡았다. 아래 표는 이 흐름을 세 가지 축에서 보여준다.
