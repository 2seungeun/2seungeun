...
아래 그림은 연구자마다 하나의 대표 출발 아키텍처와 하나의 로보틱스 역할을 부여해 센 것이다. 그림을 읽는 데 필요한 코드만 짧게 정리하면 다음과 같다.

| 코드 | 짧은 의미 |
|---|---|
| G1 | 비디오 생성 |
| G2 | 미래 비디오·상태 예측 |
| G3 | 비디오와 action 공동 생성 |
| U1 | 행동·사건·의미 이해 |
| U2 | 모션·트래킹·상호작용 이해 |
| U3 | latent representation 예측 |
| VL | video-language 정렬·추론 |
| A | action만 생성 |
