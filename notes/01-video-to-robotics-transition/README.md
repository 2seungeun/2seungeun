...
처음에는 “비디오를 하던 사람들이 요즘 로보틱스로 많이 넘어간 것 같다”는 인상에서 시작했다. Agrim Gupta가 가장 먼저 떠올랐지만, 한 사람을 출발점으로 목록을 만들면 그 사람이 실제 중요도보다 앞에 놓이기 쉽다. 그래서 이름을 나열하는 대신 비디오 쪽의 출발 아키텍처, 로보틱스에서 맡는 역할, 대표 논문의 영향력을 따로 기록했다.
...
엄밀한 의미의 “분야 전환”은 생각보다 많지 않았다. 소속이나 연구 주제가 비디오에서 로보틱스로 뚜렷하게 바뀐 경우는 57명 중 2명이었다. 반면 두 분야에서 중요한 작업을 했거나, 비디오 아키텍처를 로봇 문제에 직접 옮긴 **Bridger**는 42명이었다. 따라서 이 현상을 설명할 때는 ‘대규모 이직’보다 ‘연구 경계의 결합’이라고 부르는 편이 정확해 보인다.

![연구자 전환 유형](assets/researcher_transition_types.png)

또 하나는 비디오 생성과 비디오 이해가 로보틱스로 들어가는 경로가 꽤 다르다는 점이다.

- 비디오 생성 계열은 미래 장면이나 상태를 만들기 때문에 월드 시뮬레이션과 계획으로 이어지는 경우가 많았다.
- 모션·트래킹 중심의 비디오 이해는 로봇의 인지, 어포던스, 비디오 조건부 정책으로 연결됐다.
- V-JEPA 계열처럼 픽셀이 아니라 latent representation을 예측하는 모델은 생성 모델과 구분했다. 이쪽은 latent world model과 planning에 가깝다.
- Video-language는 다시 별도 축으로 두었다. 영상과 언어를 정렬해 얻은 표현이 reward, reasoning, policy conditioning에 쓰이는 경우다. VLA처럼 곧바로 action을 출력하는 모델과도 구분했다.
